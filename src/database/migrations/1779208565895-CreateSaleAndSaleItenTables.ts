...
export class CreateSaleAndSaleItenTables1779208565895 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "sale_item" ("id" varchar PRIMARY KEY NOT NULL, "created_at" datetime NOT NULL DEFAULT (datetime('now')), "updated_at" datetime DEFAULT (datetime('now')), "deleted_at" datetime, "quantity" integer NOT NULL, "unitPrice" integer NOT NULL, "subTotal" integer NOT NULL, "sale_id" varchar NOT NULL, "product_id" varchar NOT NULL)`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `INSERT INTO "temporary_sale_item"("id", "created_at", "updated_at", "deleted_at", "quantity", "unitPrice", "subTotal", "sale_id", "product_id") SELECT "id", "created_at", "updated_at", "deleted_at", "quantity", "unitPrice", "subTotal", "sale_id", "product_id" FROM "sale_item"`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `INSERT INTO "sale_item"("id", "created_at", "updated_at", "deleted_at", "quantity", "unitPrice", "subTotal", "sale_id", "product_id") SELECT "id", "created_at", "updated_at", "deleted_at", "quantity", "unitPrice", "subTotal", "sale_id", "product_id" FROM "temporary_sale_item"`,
    );
